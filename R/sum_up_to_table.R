...
CheckDependencies(c("dplyr", "data.table", "optparse"))
...
adhocParseArgs <- function(verbose.print = T) {
    ...
    option_list <- list(
        make_option(
            ...
            type = "character",
            default = ".",
            help = " directory of quantification result of serial samples"
        ),
        make_option(
            c("-o", "--output"),
            type = "character",
        ...
        make_option(
            ...
            type = "character",
            default = NULL,
            help = "tabulate file contains host information of each transcript"
        )
    )
...
export.pattern <-
    expand.grid(c(str.circ, str.linc, str.mrna),
                c(str.tpm, str.count),
                c(str.iso, str.gene))

on_each_pattern <-
    function(source.entry.and.unit.exp.and.unit.entry,
             df.exp.data) {
        ...
        path.full <-
            paste(
                report.path,
                "/",
                paste(source.entry, "_", unit.exp, "_", unit.entry, ".tab", sep = ""),
                sep = ""
            )
        ...
        df.exp.data %>% adhocSumUp(source.entry, unit.exp, unit.entry) %>% data.table::fwrite(file = path.full,
                                                                                              sep = "\t",
                                                                                              row.names = F)
